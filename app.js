...
function buildDistributionSuggestion(){const plan=activePlan();const current=plan.assignments||{};const employees=[...state.employees];const candidateMap=new Map();state.operations.forEach(op=>{candidateMap.set(op.id,employees.filter(emp=>isApto(skill(emp.id,op.id))));});const operations=[...state.operations].sort((a,b)=>{const ac=candidateMap.get(a.id).length,bc=candidateMap.get(b.id).length;return ac-bc||(current[a.id]?-1:1)||(current[b.id]?1:-1)||a.code.localeCompare(b.code,'pt-BR');});const employeeToOperation=new Map();const operationToEmployee=new Map();function candidateOrder(op){return [...candidateMap.get(op.id)].sort((a,b)=>{const ac=current[op.id]===a.id?1:0,bc=current[op.id]===b.id?1:0;const as=skillScore(skill(a.id,op.id)),bs=skillScore(skill(b.id,op.id));return bc-ac||bs-as||a.name.localeCompare(b.name,'pt-BR');});}function reassign(opId,seenEmployees,seenOperations){if(seenOperations.has(opId))return false;seenOperations.add(opId);const op=state.operations.find(item=>item.id===opId);if(!op)return false;for(const employee of candidateOrder(op)){if(seenEmployees.has(employee.id))continue;seenEmployees.add(employee.id);const previousOperationId=employeeToOperation.get(employee.id);if(!previousOperationId||reassign(previousOperationId,seenEmployees,seenOperations)){employeeToOperation.set(employee.id,opId);operationToEmployee.set(opId,employee.id);return true;}}return false;}operations.forEach(op=>reassign(op.id,new Set(),new Set()));const rows=state.operations.map(op=>{const employeeId=operationToEmployee.get(op.id)||'';return {opId:op.id,employeeId,level:employeeId?skill(employeeId,op.id):'',currentEmployeeId:current[op.id]||'',qualifiedCount:candidateMap.get(op.id).length};});const assignments={};operationToEmployee.forEach((employeeId,opId)=>{assignments[opId]=employeeId;});return {assignments,rows};}
function renderSuggestionPreview(){const box=document.querySelector('#suggestionPreview');if(!box)return;if(!pendingSuggestion){box.classList.add('hidden');box.innerHTML='';return;}const assigned=pendingSuggestion.rows.filter(r=>r.employeeId).length;const changes=pendingSuggestion.rows.filter(r=>r.employeeId!==r.currentEmployeeId).length;const missing=pendingSuggestion.rows.filter(r=>!r.employeeId);const trainingGaps=missing.filter(row=>row.qualifiedCount===0);const capacityGaps=missing.filter(row=>row.qualifiedCount>0);const missingList=(rows,reason)=>rows.map(row=>{const op=state.operations.find(o=>o.id===row.opId);return `<span><b>${escapeHtml(op?.code||'Step')}</b> · ${reason}</span>`}).join('');const alert=missing.length?`<div class="suggestion-alert"><strong>⚠ Maior cobertura possível encontrada</strong><span>${missing.length} step(s) continuam sem alocação porque não existe combinação viável com a equipe atual.</span>${trainingGaps.length?`<div class="suggestion-gap-group"><b>Capacitação necessária</b><div class="suggestion-missing-list">${missingList(trainingGaps,'nenhum funcionário apto (Titular ou Nível 3)')}</div></div>`:''}${capacityGaps.length?`<div class="suggestion-gap-group"><b>Equipe insuficiente para a cobertura atual</b><div class="suggestion-missing-list">${missingList(capacityGaps,'funcionários aptos já alocados em outros steps')}</div></div>`:''}<small>Cadastre novas skills, capacite funcionários ou aumente a equipe para cobrir os steps restantes.</small></div>`:`<div class="suggestion-success">✓ Todos os steps têm um montador apto disponível.</div>`;box.classList.remove('hidden');box.innerHTML=`<div class="suggestion-head"><div><strong>Sugestão automática</strong><span>${assigned} de ${pendingSuggestion.rows.length} steps preenchidos · ${changes} alterações previstas</span></div><div class="suggestion-actions"><button class="btn secondary" id="cancelSuggestionBtn">Cancelar</button><button class="btn primary" id="applySuggestionBtn">Aplicar sugestão</button></div></div>${alert}<div class="suggestion-list">${pendingSuggestion.rows.map(row=>{const op=state.operations.find(o=>o.id===row.opId);const current=state.employees.find(e=>e.id===row.currentEmployeeId);const selected=state.employees.find(e=>e.id===row.employeeId);const info=selected?levelInfo(row.level):null;return `<div class="suggestion-row"><strong>${escapeHtml(op?.code||'Step')}</strong><span>${current?`Atual: ${escapeHtml(current.name)}`:'Atual: vazio'}</span><span>${selected?`Sugestão: <b>${escapeHtml(selected.name)}</b> · ${info[1]}`:'Sem montador apto disponível'}</span></div>`}).join('')}</div>`;document.querySelector('#cancelSuggestionBtn').onclick=()=>{pendingSuggestion=null;renderSuggestionPreview();};document.querySelector('#applySuggestionBtn').onclick=()=>{activePlan().assignments={...pendingSuggestion.assignments};pendingSuggestion=null;save();renderLineBoard();flash('Distribuição sugerida aplicada');};}
