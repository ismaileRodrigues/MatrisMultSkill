...
function saveWeeklySchedule(empId,day,opId){const plan=activePlan();const eligible=eligibleOperations(empId).some(o=>o.id===opId);if(opId&&!eligible){flash('Ação bloqueada: o funcionário não é apto para este step');renderLineBoard();return false;}const error=weeklyValidation(plan,empId,day,opId);if(error){flash('Ação bloqueada: '+error);renderLineBoard();return false;}plan.weeklySchedules=plan.weeklySchedules||{};plan.weeklySchedules[empId]=plan.weeklySchedules[empId]||{};if(opId)plan.weeklySchedules[empId][day]=opId;else delete plan.weeklySchedules[empId][day];if(!save())return false;renderLineBoard();flash('Programação semanal salva');return true;}
